...
#import "TickWhiteView.h"
#import "QCMethod.h"

@interface TickWhiteView ()<CAAnimationDelegate>
...
- (void)setupProperties{
	...
	self.ck_gray = [UIColor colorWithRed:0.404 green: 0.393 blue:0.53 alpha:1];
}
...
- (void)resetLayerPropertiesForLayerIdentifiers:(NSArray *)layerIds{
	...
	if(!layerIds || [layerIds containsObject:@"path"]){
		...
		path.fillColor   = nil;
		path.strokeColor = [UIColor colorWithRed:0.404 green: 0.393 blue:0.53 alpha:1].CGColor;
		path.lineWidth   = 4;
	}
...
- (void)addTickViewAnimationTotalDuration:(CFTimeInterval)totalDuration completionBlock:(void (^)(BOOL finished))completionBlock{
	...
	ovalFillColorAnim.values    = @[(id)self.white.CGColor, 
		 (id)self.white.CGColor];
